...
完成分析报告后,系统会自动将完整的投资备忘录保存为Markdown格式文件。

### 保存位置

报告将保存到用户主目录,文件名格式为:
```
~/${股票代码}_${公司名称}_Investment_Analysis.md
```

例如:
- `~/NVDA_NVIDIA_Investment_Analysis.md`
- `~/META_Meta_Platforms_Investment_Analysis.md`
- `~/AAPL_Apple_Investment_Analysis.md`

### 文件内容

保存的Markdown文件包含完整的分析报告,包括:
- 执行摘要与TL;DR
- Key Forces分析
- A-P各模块的深度分析
- 估值矩阵
...
报告保存完成后,会向用户显示保存路径,例如:
> 报告已保存至 `/Users/mac/NVDA_NVIDIA_Investment_Analysis.md`
